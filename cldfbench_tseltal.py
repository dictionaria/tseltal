...
from pydictionaria.formats.sfm_lib import Database as SFM, Entry
from pydictionaria import sfm2cldf

from cldfbench import CLDFSpec, Dataset as BaseDataset

EID_PATTERN = re.compile('(?P<eid>TSE[0-9]+)')
...
def preprocess(entry):
    ...
    for marker, content in entry:
        if marker == 'sy':
            eids = [m.group('eid') for m in EID_PATTERN.finditer(content)]
            if eids:
                new.append(('sy', ' ; '.join(eids)))
            if EID_PATTERN.sub('', content).strip():
                new.append(('sydial', content))
        else:
